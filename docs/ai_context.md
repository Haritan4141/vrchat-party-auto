...
- サブスキル座標は `subSkillMoveX := 300`, `subSkillMoveY := -50`。メインスキル1暫定座標は `mainSkillMoveX := 80`, `mainSkillMoveY := -46`
...
- `SmoothMouseMoveRel()` は累積目標位置との差分で移動する。短い移動時間で分割数が少ない場合でも、`mainSkillMoveY := -46` などの設定値どおりの合計移動量になる
