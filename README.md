...
`vrchat_party_macro_click_repeater.ahk` だけはダンジョンへの入場やセンターポジションが不要です。クリックしたい位置へカーソルを合わせてから `F8` を押してください。
...
実行するAHKに転生や売却ロジックがない場合、対応する間隔欄は非活性になります。転生と売却のどちらもない場合は、ダンジョンボタン位置も非活性になります。`vrchat_party_macro_skill_infinite_dungeon.ahk` はダンジョンクリア間隔も使わないため、ダンジョンクリア間隔、転生間隔、売却間隔、ダンジョンボタン位置が非活性になります。`vrchat_party_macro_click_repeater.ahk` はクリック連打間隔だけが有効になります。

使い方:

1. 実行するAHKファイルを選択
2. 使用する間隔を入力（通常設定は秒、クリック連打間隔はミリ秒）
3. ダンジョンボタン位置を選択
4. `適用して実行` を押す
...
クリック連打間隔は `vrchat_party_macro_common_click_repeater_config.ahk` に保存されます。このファイルは各PCのローカル設定としてGUIが自動生成し、Git管理対象外にしています。ファイルがない初回起動時は100msを使用します。
...
各ダンジョンマクロはAutoスキル位置を中心位置として扱います。再入場、逃げる、転生、売却、ダンジョン選択、メインスキル、サブスキルは、Autoスキル位置から一時的に移動してクリックし、クリック後はAutoスキル位置へ戻ります。例外として、`vrchat_party_macro_skill_infinite_dungeon.ahk` は初回だけAutoスキル位置を起点にし、その後はメインスキル位置とサブスキル位置を直接行き来します。`vrchat_party_macro_click_repeater.ahk` は独立したクリック連打用で、センターポジションを使用しません。
...
`vrchat_party_macro_click_repeater.ahk`

F8開始時にカーソルを置いている位置を、GUIの `クリック連打間隔 (ms)` で連打します。マクロ自身はカーソルを移動しません。クリック押下時間は20ms固定で、設定値は押下開始から次の押下開始までの総間隔です。設定可能な最小値は20msです。
